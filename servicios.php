...
// 2) Si BD falla → usar fallback completo (incluye imagen)
if (empty($servicios)) {
  $servicios = $fallback_servicios;
} else {
  // 3) BD OK → SIEMPRE asignar imagen por ORDEN (índice)
  ...
  <!-- ✅ TITULO SECCIÓN SERVICIOS -->
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-8">
      ...
      <?php foreach ($servicios as $s): ?>
        <div class="group relative h-[460px] sm:h-[520px] overflow-hidden border border-white/10 bg-navy-blue shadow-2xl">

          <img
            alt="<?= htmlspecialchars($s["nombre"], ENT_QUOTES, "UTF-8"); ?>"
            class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
            src="<?= htmlspecialchars($s["imagen"], ENT_QUOTES, "UTF-8"); ?>" />
          ...
          <div class="absolute inset-0 p-6 sm:p-10 flex flex-col justify-end">
            ...
            <p class="text-gray-300 text-sm leading-relaxed mb-8">
              <?= htmlspecialchars($s["descripcion"], ENT_QUOTES, "UTF-8"); ?>
            </p>

            <a href="contacto.php"
              class="bg-primary-red hover:bg-white hover:text-navy-blue transition-all text-white font-black text-[10px] tracking-widest uppercase py-3 px-6 sm:py-4 sm:px-8 self-start rounded-sm">
              Cotizar
            </a>
          </div>
        </div>
